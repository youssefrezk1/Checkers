...
from checkers.agents.explainer_agent import explainer_agent as ranker_agent
...
def _build_suspicious_mismatch_record(
    ...
    explainer_menu = decision_snapshot.get("explainer_filtered_menu") or []
    legal = explainer_menu if explainer_menu else (decision_snapshot.get("legal_moves") or [])
...
def _extract_red_decision(snapshot: dict[str, Any], debug: dict[str, Any]) -> RedDecisionStats:
    explainer_menu = snapshot.get("explainer_filtered_menu") or []
    legal_moves = explainer_menu if explainer_menu else (snapshot.get("legal_moves") or [])
...
def _run_red_ply(
    ...
    *,
    ...
    decision_snapshot["explainer_filtered_menu"] = (
        list(st.explainer_filtered_menu or []) if getattr(st, "explainer_filtered_menu", None) else None
    )
...
def run_batch(
    ...
    *,
    ...
    red_decisions_with_explainer_filtered_menu = 0
    red_decisions_missing_explainer_filtered_menu = 0
    chosen_move_unmatched_on_explainer_filtered_menu = 0
        ...
        while not state.get("game_over") and (state.get("turn_number", 0) < max_plies):
            if state["current_player"] == RED:
                ...
                if decision is not None:
                    ...
                    if explainer_menu:
                        red_decisions_with_explainer_filtered_menu += 1
                        if not _chosen_matches_menu(explainer_menu, chosen_move):
                            chosen_move_unmatched_on_explainer_filtered_menu += 1
                    else:
                        red_decisions_missing_explainer_filtered_menu += 1
                    ...
                    # Safety audit
                    menu = explainer_menu if explainer_menu else (decision_snapshot.get("legal_moves") or [])
    ...
    summary = {
        ...
        "debug_explainer_menu_presence": {
            "red_decisions_with_explainer_filtered_menu": red_decisions_with_explainer_filtered_menu,
            "red_decisions_missing_explainer_filtered_menu": red_decisions_missing_explainer_filtered_menu,
            "chosen_move_unmatched_on_explainer_filtered_menu": chosen_move_unmatched_on_explainer_filtered_menu,
