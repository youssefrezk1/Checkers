...
class CheckersState(BaseModel):
    ...
    # ── Chosen move (sole authority: proposer_agent) ──────────────────────────
    # chosen_move is WRITTEN ONLY by proposer_agent. explainer_agent
    ...
    # Natural-language explanation of the chosen move, produced by explainer_agent.
    ...
    # Written by proposer_agent; cleared by state_manager each turn.
    ...
    # Passed to explainer_agent for comparative explanation only — never used to
    ...
    # to explainer_agent when it generates its explanation. Used purely for
    ...
    explainer_filtered_menu: Optional[list[dict[str, Any]]] = Field(default=None)

    # ── Legacy explainer retry / thesis metrics ────────────────────────────────
    # Retained for log-schema compatibility with the pre-simplified pipeline.
    # In the proposal-authoritative pipeline the explainer performs no decision
    # work and never retries or falls back, so these counters stay at 0.
    explainer_retry_count: int = Field(default=0)
    explainer_failure_count: int = Field(default=0)
    explainer_fallback_count: int = Field(default=0)
    explainer_retry_budget: int = Field(default=3)
    ...
    # ── Format error tracking (legacy, always 0 in simplified pipeline) ─────
    ...
    # Whole-position balance classification used by explainer_agent for adversity-
    ...
    # ── Symbolic scoring output (written by scorer_agent each turn) ──────────
    # symbolic_scored_moves: all legal moves sorted best-first.
    ...
    # Thesis instrumentation — set by scorer_agent and explainer_agent.
    llm_invoked: bool = Field(default=False)
    llm_agreed_with_symbolic_best: Optional[bool] = Field(default=None)
    proposer_diagnostics: Optional[dict[str, Any]] = Field(default=None)
    # Structured diagnostics emitted by explainer_agent when it generates the
    ...
    # evaluation schemas remain stable. Persists until the next explainer_agent
    # call (state_manager does NOT clear this field between turns).
    explainer_diagnostics: Optional[dict[str, Any]] = Field(default=None)
    # Full compute_move_facts() output for the proposal-chosen move.
    # Captured by explainer_agent from chosen_move["facts"] for evaluation export;
