...
# ── Deterministic single-move selector ────────────────────────────────────────
# New public API added alongside select_proposal_candidates (which is preserved
# for backward compatibility).  Called by deterministic_proposal_node.
...
def select_best_move(
    ...
    strategic_context :
        Optional dict from CheckersState.strategic_context.  Used only for
        metadata/diagnostics — selection is purely minimax rank-1.
    ...
    """
    if not scored_moves:
        # Defensive: should never happen (scorer returns [] only when no
        # legal moves, and the graph would not reach proposal in that case).
    ...
    logger.info(
        ...
        "chosen_score=%.2f gap=%s game_phase=%s score_state=%s path=%s",
        len(scored_moves),
        chosen_score,
